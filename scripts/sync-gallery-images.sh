...
usage() {
  cat <<'EOF'
Usage: scripts/sync-gallery-images.sh <source.zip|source-dir> [output-dir] [manifest-file]

Converts gallery source images to web-optimized WebP assets and writes a
TypeScript manifest consumed by the gallery page.

Defaults:
  output-dir:    public/gallery
  manifest-file: src/content/gallery-images.generated.ts
...
MANIFEST_FILE="${3:-src/content/gallery-images.generated.ts}"
...
touch "$METADATA_TSV"
mkdir -p "$OUTPUT_DIR" "$(dirname "$MANIFEST_FILE")"
...
python3 - "$METADATA_TSV" "$MANIFEST_FILE" <<'PY'
...
for line in metadata_path.read_text(encoding="utf-8").splitlines():
    src, full_src, width, height, full_width, full_height, original_name = line.split("\t")
    rows.append(
        {
            "src": src,
            "fullSrc": full_src,
...
for row in rows:
    lines.extend(
        [
            "  {",
            f"    src: {json.dumps(row['src'])},",
            f"    fullSrc: {json.dumps(row['fullSrc'])},",
            f"    width: {row['width']},",
            f"    height: {row['height']},",
            f"    fullWidth: {row['fullWidth']},",
            f"    fullHeight: {row['fullHeight']},",
            f"    originalName: {json.dumps(row['originalName'], ensure_ascii=False)},",
            "  },",
        ]
    )
...
echo "Gallery manifest written to $MANIFEST_FILE"
echo "Done."
